...
import { NowPlayingNextArrow, NowPlayingPrevArrow } from './CustomSliderArrow';

import styles from './styled/now-playing-movies.module.css';

export default function NowPlayingMoviesSlider({ slides }) {
  const settings = {
    infinite: false,
    speed: 500,
    slidesToScroll: 4,
    slidesToShow: 5,
    nextArrow: <NowPlayingNextArrow />,
    prevArrow: <NowPlayingPrevArrow />,
  };
  return (
    <Slider {...settings}>
      {slides.map((slide) => (
